...
class TicketMachinePurchaseMqttIntegrationTests {
    ...
    @Test
    void shouldExecuteARechargeRequestedByAnAuthenticatedTicketMachine() {
        AuthenticatedMqttMessageRouter router = mock(AuthenticatedMqttMessageRouter.class);
        TicketMachineRechargeService rechargeService = mock(TicketMachineRechargeService.class);
        MqttTicketRechargeResponsePublisher publisher = mock(MqttTicketRechargeResponsePublisher.class);
        Purchase purchase = mock(Purchase.class);
        Ticket ticket = mock(Ticket.class);
        TicketMachineRechargeResult result = new TicketMachineRechargeResult(
                purchase, ticket, "RMM:TICKET:1:signed"
        );
        when(rechargeService.recharge(any(), any())).thenReturn(result);

        new MqttTicketOperationEventReceiver(
                router, mock(DeviceEventIngress.class), new ObjectMapper(),
                mock(TicketMachinePurchaseService.class), mock(TicketValidationService.class),
                mock(MqttTicketValidationResponsePublisher.class), rechargeService, publisher
        );
        ...
        String messageId = "73bb91e8-b263-41e8-aa8f-b791480110b3";
        String payload = """
                {
                  "schemaVersion": 1,
                  "messageId": "%s",
                  ...
                  "payload": {
                    "rechargeReference": "9561ad31-6273-42d9-b76f-2dabb0b60955",
                    "qrValue": "RMM:TICKET:1:signed",
                    "paymentMethod": "SIMULATED",
                    "paidAmount": 5.00,
                    "currency": "EUR",
                    "configuration": { "trips": 5 }
                  }
                }
                """.formatted(messageId);
        ...
        consumerCaptor.getValue().accept(new AuthenticatedMqttMessage(
                machine,
                "rmm/v1/devices/RMM-TM-ST001-01/requests/recharges",
                payload.getBytes(StandardCharsets.UTF_8)
        ));

        ArgumentCaptor<TicketMachineRechargeRequest> request =
                ArgumentCaptor.forClass(TicketMachineRechargeRequest.class);
        verify(rechargeService).recharge(org.mockito.ArgumentMatchers.eq(41L), request.capture());
        assertThat(request.getValue().trips()).isEqualTo(5);
        assertThat(request.getValue().paidAmount()).isEqualByComparingTo(new BigDecimal("5.0"));
        verify(publisher).publish(machine, messageId, result);
    }
