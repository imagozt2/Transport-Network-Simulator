...
describe('Stations', () => {
  it('should render station state, line colors and a live mm:ss countdown', async () => {
    await TestBed.configureTestingModule({
      imports: [Stations],
      providers: [provideRouter([]), { provide: StationOperationsService, useValue: { getOperations: () => of(response) } }],
    }).compileComponents();
    ...
    expect(summaryCards.map((card) => card.querySelector('span')?.textContent?.trim())).toEqual(['Estaciones', 'Transbordos', 'Máquinas de billetes', 'Validadores de entrada', 'Validadores de salida']);
    expect(summaryCards.map((card) => card.querySelector('strong')?.textContent?.trim())).toEqual(['1', '0', '1', '1', '1']);
    ...
    expect(Array.from(filterGroups.item(1).querySelectorAll('button')).map((button) => button.textContent?.trim())).toEqual(['Todas', '1 línea', '2 líneas', '3 o más']);
  ...
  it('should initialize the line filter from the URL', async () => {
    ...
    expect((fixture.nativeElement.querySelector('.filter-select select') as HTMLSelectElement).value).toBe('L3');
    fixture.destroy();
  });
  ...
  it('should focus and expand a station received from a contextual link', async () => {
    ...
    expect(fixture.nativeElement.querySelector('.station-card.expanded .station-code')?.textContent).toContain('STB');
    fixture.destroy();
  });
});
